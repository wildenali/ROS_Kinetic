...
def adaptive_thresholding(gray_image, threshol_value):
    # adaptive_threshold_image = cv2.adaptiveThreshold(gray_image, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, threshol_value, 2)
    adaptive_threshold_image = cv2.adaptiveThreshold(gray_image, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, threshol_value, 2)
    cv2.imshow("Adaptive Threshold Image", adaptive_threshold_image)

def main():
    # image_name = "image/shapes.png"
    image_name = "image/tomato.jpg"
    as_gray = True
    # threshol_value = 155
    threshol_value = 200
