...
protected:
  ros::NodeHandle _nh;
  actionlib::SimpleActionServer<ba_my_robot_msgs::MoveRobotAction> _as;
  int _current_position;

public:
  MoveRobotServer(): _as(_nh, "/move_robot", boost::bind(&MoveRobotServer::onGoal, this, _1), false), _current_position(50) {
    _as.start();
    ROS_INFO("Move Robot server has been started");
  }

  void sendFeedback() {
    ba_my_robot_msgs::MoveRobotFeedback feedback;
    feedback.current_position = _current_position;
    _as.publishFeedback(feedback);
  }

  void onGoal(const ba_my_robot_msgs::MoveRobotGoalConstPtr &goal) {
    ...
    ba_my_robot_msgs::MoveRobotResult result;
