...
protected:
ros::NodeHandle _nh;
actionlib::SimpleActionClient<ba_my_robot_msgs::CountUntilAction> _ac;
...
void sendGoal() {
  ba_my_robot_msgs::CountUntilGoal goal;
  goal.max_number = 12;
...
void doneCb(const actionlib::SimpleClientGoalState& state, const ba_my_robot_msgs::CountUntilResultConstPtr &result) {
  ROS_INFO("Finished in state: %s", state.toString().c_str());
  ROS_INFO("Count result: %d", (int)result->count);
}

void activeCb() {
  ROS_INFO("Goal just wen active");
}

void feedbackCb(const ba_my_robot_msgs::CountUntilFeedbackConstPtr &feedback) {
  ROS_INFO("Feedback received. Precentage: %lf", feedback->percentage);
}
