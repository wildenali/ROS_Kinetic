...
def go_to_goal(x_goal, y_goal):
    ...
    while True:
        K_linear = 0.5
        distance = abs(math.sqrt(((x_goal - x) ** 2) + ((y_goal - y))))
        ...
        velocity_publisher.publish(velocity_message)
        print 'x= ', x, 'y= ', y, 'jarak= ', distance

        if distance < 0.01:
            print 'udah sampe coy'
            break

def setDesiredOrientation(desired_angle_radians):
    relative_angle_degree = desired_angle_radians - yaw
    if relative_angle_degree < 0:
        clockwise = 1
    else:
        clockwise = 0
    print relative_angle_degree
    print desired_angle_radians
    rotate(30, math.degrees(abs(relative_angle_degree)), clockwise)

# gridClean dan spiralClean belum selesai
# def gridClean():
#     desired_pose = Pose()
#     desired_pose.x = 1
#     desired_pose.y = 1
...
if __name__ == '__main__':
    try:
        ...
        # ini buat nge test turtle ke goal yang telah ditentukan
        go_to_goal(1.0, 1.0)

        # ini buat ngetest program setDesiredOrientation()
        setDesiredOrientation(math.radians(90))
        setDesiredOrientation(math.radians(0))
        setDesiredOrientation(math.radians(270))
        setDesiredOrientation(math.radians(90))
        setDesiredOrientation(math.radians(180))
        setDesiredOrientation(math.radians(90))
        setDesiredOrientation(math.radians(360))
